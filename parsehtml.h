...
namespace{
const char* hrefFmt = R"(<a href="http://qcrt.hexagonmetrology.com%1%">%2%</a>)";
}
        ...
        try
        {
            ...
            for (const auto& child : elements)
            {
                ...
                if (auto e = dynamic_cast<const xmlpp::Element*>(items[5]->find(".//font").front()))
                {
                    if (auto text_node = e->get_first_child_text())
                    {
                        caseDetails.caseResult = text_node->get_content();
                    }
                }

                if (auto e = dynamic_cast<const xmlpp::Element*>(items[11]->find(".//font").front()))
                {
                    if (auto text_node = e->get_first_child_text())
                    {
                        caseDetails.caseTime = text_node->get_content();
                    }
                }

                if (auto e = dynamic_cast<const xmlpp::Element*>(items[12]->find(".//font/a").front()))
                {
                    std::string content, ref;
                    if (auto text_node = e->get_first_child_text())
                    {
                        content = text_node->get_content();
                    }

                    if (auto attr = e->get_attribute("href"))
                    {
                        ref = attr->get_value();
                    }

                    if (!content.empty() && !ref.empty())
                    {
                        caseDetails.caseDet = boost::str(boost::format(hrefFmt) % ref %content);
                        //BOOST_LOG(queryLog::get()) << caseDetails.caseDet << std::endl;
                    }
                }

                if (auto e = dynamic_cast<const xmlpp::Element*>(items[13]->find(".//font/a").front()))
                {
                    std::string content, ref;
                    if (auto text_node = e->get_first_child_text())
                    {
                    ...
                    if (!content.empty() && !ref.empty())
                    {
                        caseDetails.caseSeq = boost::str(boost::format(hrefFmt) % ref %content);
                        //BOOST_LOG(queryLog::get()) << caseDetails.caseSeq << std::endl;
                    }
                }

                if (auto e = dynamic_cast<const xmlpp::Element*>(items[14]->find(".//font").front()))
                {
                    if (auto text_node = e->get_first_child_text())
                    {
                        caseDetails.caseMachine = text_node->get_content();
                    }
                }

                if (num != 0 && !caseDetails.caseResult.empty())
                {
                    msg.results.emplace(num, caseDetails);
                }
            }
